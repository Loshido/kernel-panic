...
var points = [];
var svg = (width2, height2, points2) => {
  const path = points2.map(([x, y]) => `L${x} ${y}`).join(" ") + " z";
...
var line = () => {
  ...
  for (let i = 0; i < n2 - 1; i++) {
    ...
    const y = Math.floor(points2[i][1] + rand(50, 25) * facteur);
    points2.push([
      x,
      y
    ]);
...
var reload = () => {
  const parent = document.querySelector("div.montagne > svg");
...
// src/client/index/group.ts
var maxheight2 = Math.floor(window.innerHeight / 3 * 2);
var groups = [];
...
var Group = class _Group {
  ...
  update(score) {
    ...
    let y = _Group.nearest_height(this.x);
    groups.forEach((g) => {
      if (Math.abs(g.y - y) <= 20 && Math.abs(g.x - this.x) <= 20) {
        y += 80;
      }
    });
    this.y = y;
...
var journal_default = async () => {
  ...
  for await (const chunk of stream.values({
    ...
    lignes.split("\n").map((l) => [
      ...
      n++;
      if (n >= 30) {
        journal.querySelector("main > section:nth-child(1) > p:nth-child(2)")?.remove();
      }
