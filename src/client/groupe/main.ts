...
const buttonImage = document.querySelector('header > label > input') as HTMLInputElement
...
const enregistrer = document.querySelector('footer > button') as HTMLButtonElement
    ...
    const m: [string, string][] = membres
        ...
        .map(membre => {
            if(membre.nom.length === 0) {
                membre.element?.querySelector('div:nth-child(1)')?.classList.toggle('error', true)
                erreur = true
            }
            if(membre.prenom.length === 0) {
                membre.element?.querySelector('div:nth-child(2)')?.classList.toggle('error', true)
                erreur = true
            }
...
enregistrer.addEventListener('click', () => {
    ...
    if(!valide || membres.length === 0) {
        validation = false
        indication += 'Veuillez remplir les noms et prénoms de tous les membres.\n'
    }
    ...
    if(prompt("Entrez 'oui' pour créer le groupe " + groupe.innerText) !== 'oui') return
