...
export default (): HTMLElement => {
    ...
    ;[
        {
            ...
            async action() {
                ...
                const response = await fetch(
                    `/admin/avancer?t=${admin}&g=${groupe}`,
                )
        ...
        },
        {
            ...
            async action() {
                if (!groupe) return
                const response = await fetch(
                    `/admin/reculer?t=${admin}&g=${groupe}`,
                )
                if (response.ok) close()
            },
        },
    ...
    const montagne = document.querySelector('div.montagne') as
        | HTMLElement
        | null
    ...
    document.addEventListener('click', (event) => {
        ...
        if (
            !(context.isSameNode(target) || context.contains(target)) && opened
        ) {
            close()
        }
    })
